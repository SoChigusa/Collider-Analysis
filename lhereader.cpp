...
LHEReader::LHEReader(std::string fname) {
  ...
  v = split(buf_pb, ' ');
  xsec = stod(v[0]);
  std::cout << fname << ": # of events = " << nevent
	    << ", sigma = " << xsec << " pb." << std::endl;

  ev.resize(nevent);
  int n = 0;
  std::string buf_ev;
  while(findStr(ifs, buf_ev, "<event>")) saveEvent(ifs, n);
...
int LHEReader::findStr(std::ifstream &ifs, std::string &buf, const std::string &f) {
  getline(ifs, buf);
...
void LHEReader::nextNline(std::ifstream &ifs, std::string &buf, const int n) {
  for(int i = 0; i < n; i++) { getline(ifs, buf); }
}
