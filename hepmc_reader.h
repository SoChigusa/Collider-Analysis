...
public:
  ...
  void addDecayBranch(const int b) {
    bid.push_back(b);
  } // NOTE: only one branch for one decay particle in an event shown
  void analyze();
  ...
  void displayDBranch(std::unordered_map<std::string, int> &db) {
    db = dbranch;
  }
};
...
double invMass(const Particle &p1, const Particle &p2) {
  return sqrt(
      (p1.E + p2.E) * (p1.E + p2.E) - (p1.px + p2.px) * (p1.px + p2.px) -
      (p1.py + p2.py) * (p1.py + p2.py) - (p1.pz + p2.pz) * (p1.pz + p2.pz));
}
...
void HepMCReader::toParticle(Particle &arg_p, const int arg_pid,
                             const std::vector<std::string> &arg_v) {
  arg_p.pID = arg_pid;
  ...
  if (stod(arg_v[11]) != 0.)
    arg_p.Nsub =
        stod(arg_v[12]) / stod(arg_v[11]); // N-subjettiness for convenience
}
...
void HepMCReader::split(std::vector<std::string> &v, const std::string &buf,
                        char sep) {
